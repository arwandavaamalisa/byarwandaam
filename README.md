...
├── profile.jpg                               → portrait shown in the hero
└── Arwanda_Nur_Fatta_Amalisa_Resume.pdf      → downloadable résumé
```

All files sit at the **root** of the repo (no subfolder) — that's what
`index.html` expects. If you ever move `profile.jpg` or the résumé PDF into
a subfolder, update the matching `src=` / `href=` paths in `index.html`.

## Already set up

- **Résumé PDF** — `Arwanda_Nur_Fatta_Amalisa_Resume.pdf` is linked from
  both the hero "Download Résumé" button and the Contact section. Upload a
  newer version any time — just keep the same filename, or update the two
  `href="Arwanda_Nur_Fatta_Amalisa_Resume.pdf"` references in `index.html`.
- **Portrait photo** — `profile.jpg` is displayed beside the hero text.
  Swap the file any time — keep the filename `profile.jpg`, or update the
  `<img src="profile.jpg">` line in `index.html`.
